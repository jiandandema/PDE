...
PetscErrorCode ComputeLfunction(DMDALocalInfo *info, Field **x, Field **f, Field **z, AppCtx *user)
{
    ...
    for (i = xints; i < xinte; i++)
    {
      if (j > 0 && j < info->my - 1)
      {
        ...
        for (k = 0; k < user->Q; k++)
        {
          ...
          f[j][i].f[k] = -epsilon * Dfx - epsilon * Dfy + (CalFeq(k, sumrho, sumu, sumv, user) - x[j][i].f[k]) / z[j][i].f[k] + user->F * 3 * user->w[k] * user->eu[k];
          ...
          if (user->eu[k] >= 0)
          {
            ...
            Dfx = user->eu[k] * (x[j][i].f[k] - x[j][i - 1].f[k]) / dx;
          }
          else
          {
          ...
          {
            //if (j == 1)
            //  Dfy = user->ev[k] * (x[j + 2][i].f[k] - 2 * x[j + 1][i].f[k] + 3 * x[j][i].f[k] - 2 * x[j - 1][i].f[k]) / 2.0 / dy; /* upwind scheme */ /* interpolation for virtual layer */
            //else
            //  Dfy = user->ev[k] * (3 * x[j][i].f[k] - 4 * x[j - 1][i].f[k] + x[j - 2][i].f[k]) / 2.0 / dy;
            Dfy = user->ev[k] * (x[j][i].f[k] - x[j - 1][i].f[k]) / dy;
          }
          else
          {
            //if (j == info->my - 2)
    ...
    if (j == 0)
      for (i = xints; i < xinte; i++)
      {
        ...
        { /* boundary points */
          ...
          {
#if highorderext == 1
            x1[j][i].f[k] = CalFeq(k, sumrho, sumu, sumv, user) + 2 * x1[j + 1][i].f[k] - 2 * CalFeq(k, rho, u, v, user) - x1[j + 2][i].f[k] + CalFeq(k, prho, pu, pv, user); /* second order accuracy NEM */
#else
    ...
    if (j == info->my - 1)
      for (i = xints; i < xinte; i++)
      {
        if (i >= 0 && i <= info->mx - 1)
        {
          ...
          {
#if highorderext == 1
            x1[j][i].f[k] = CalFeq(k, sumrho, sumu, sumv, user) + 2.0 * (x1[j - 1][i].f[k] - CalFeq(k, rho, u, v, user)) - x1[j - 2][i].f[k] + CalFeq(k, prho, pu, pv, user);
#else
            x1[j][i].f[k] = CalFeq(k, sumrho, sumu, sumv, user) + x1[j + 1][i].f[k] - CalFeq(k, rho, u, v, user);
#endif
          }
