  ...
  for (j = info->ys; j < info->ys + info->ym; j++)
  {
    for (i = info->xs; i < info->xs + info->xm; i++)
    {
      if (j < user->Ny / 2)
      {
        ...
        phi = 0.5 * (-n + 1);
        nu = ((user->rate - 1) * phi + 1) * user->nu_l;
        x[j][i].f[0] = nu * 3.0;
      }
      else
      {
...
PetscErrorCode evolution(DMDALocalInfo *info, Vec X, Vec Y, AppCtx *user)
{
  ...
  PetscTime(&v1);
  ComputeLfunction(info, xp, x, loc_tao, user); /* compute inner point for x1 */
  xints = info->xs;
  xinte = info->xs + info->xm;
  yints = info->ys;
  yinte = info->ys + info->ym; /* same info from DMDALocalInfo *info */
  for (j = yints; j < yinte; j++)
  {
    for (i = xints; i < xinte; i++)
    {
      if (j > 0 && j < info->my - 1)
      {
        for (k = 0; k < user->Q; k++)
        {
          x[j][i].f[k] = xp[j][i].f[k] + user->stime * x[j][i].f[k]; /* refer to ComputeLfunction() */
        }
      }
    }
  }
  ComputeLBoundary(info, x, user); /* compute boundry point for x1 */

  PetscTime(&v2);
  v3 = v2 - v1;
  DMDAVecRestoreArray(info->da, X, &x); /* X1 is unrelated to x1[j][i]  */
/*
  ...
  DMDAVecGetArray(info->da, loc_x1, &loc_X1);
  ...
  for (j = yints; j < yinte; j++)
  {
    for (i = xints; i < xinte; i++)
    {
      if (j > 0 && j < info->my - 1)
      {
        for (k = 0; k < user->Q; k++)
        {
          x[j][i].f[k] = 0.5 * (xp[j][i].f[k] + loc_X1[j][i].f[k]) + 0.5 * user->stime * x[j][i].f[k];
        }
      }
    }
  }
...
PetscErrorCode SetOptions(AppCtx *user)
{
  ...
  user->rate = 50.0;
  user->nu_l = 0.02;
  user->nu_s = user->nu_l * user->rate;
  user->lidvelocity = 0.01;
  user->Re_l = user->H * user->lidvelocity / user->nu_l;
  user->Re_s = user->H * user->lidvelocity / user->nu_s;
  user->Q = 9;
  user->ep = 0.0;
  user->F = 8 * user->lidvelocity  * user->rho0 * user->nu_s;
