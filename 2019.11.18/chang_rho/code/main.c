...
int main(int argc, char **argv)
{
  ...
  InitialF(&info, user.F, &user);
  sprintf(sn, "pre_tao.dat");
  sprintf(sm, "extao.dat");
  DataSaveASCIITaoandF(&info, user.tao, sn, sm);
  sprintf(sn, "pre_F.dat");
  sprintf(sm, "exF.dat");
  DataSaveASCIITaoandF(&info, user.F, sn, sm);
  ...
  for (nits = 1; nits <= MaxN; nits++) {
    ...
    evolution(&info, user.x, user.tao, user.F, &user);
...
PetscErrorCode InitialTao(DMDALocalInfo *info, Vec X, AppCtx *user)
{
  ...
  Field          **x;
  PetscScalar	 nu;
  PetscScalar  phi, n, r, xl, yl, x0, y0;
  ...
  for (j = info->ys; j < info->ys + info->ym; j++) {
    for (i = info->xs; i < info->xs + info->xm; i++) {
      ...
      x0 = xl - 1.0/2.0;y0 = yl - 1.0/2.0;
      r = sqrt(pow(x0,2) + pow(y0,2));
      n = tanh(2.4*(r - 1.0 / 4.0) / user->W);
      phi = 0.5 * (-n + 1);
      nu = (user->rate - 1) * phi + 1;
      x[j][i].f[0] = nu * 3.0 / (user->Re * user->rho0);
      for (k = 1; k < user->Q; k++) {
        x[j][i].f[k]  = x[j][i].f[0];
      }
    }
...
PetscErrorCode InitialF(DMDALocalInfo *info, Vec X, AppCtx *user){
  PetscInt        i,j,k;
  Field           **x;
  PetscScalar     grad_nu, grad_u, grad_r, nu;
  PetscScalar     n, phi, r, xl, yl;
...
PetscErrorCode evolution(DMDALocalInfo *info, Vec X, Vec Y, Vec F, AppCtx *user)
{
  PetscInt       xints, xinte, yints, yinte, i, j, k;
  Field 	 **x, **xp, **x1, **loc_X1, **loc_tao, **loc_f;
  ...
  DMDAVecGetArray(info->da, F, &loc_f);

  PetscTime(&v1);
  ComputeLfunction(info, xp, x1, loc_tao, loc_f, user);                /* compute inner point for x1 */
  ...
  PetscTime(&v1);
  ComputeLfunction(info, loc_X1, x, loc_tao, loc_f, user);
  for (j = yints; j < yinte; j++) {
    for (i = xints; i < xinte; i++) {
      if (j > 0 && j < info->my - 1) {
        for (k = 0; k < user->Q; k++) {
  ...
  DMDAVecRestoreArray(info->da, F, &loc_f);
  VecDestroy(&X1);
  VecDestroy(&loc_xp);
  VecDestroy(&loc_x1);
