...
int main(int argc, char **argv)
{
  ...
  for (nits = 1; nits <= MaxN; nits++) {
    ...
    if (nits % 10 == 1) {
      ...
      sprintf(sn, "pre_ex%dF.dat", nits);                /* Prints information to a PetscViewer string */
      sprintf(sm, "ex%dF.dat", nits);
      DataSaveASCII(&info, user.F, sn, sm);
      sprintf(sn, "pre_ex%dFneq.dat", nits);                /* Prints information to a PetscViewer string */
      sprintf(sm, "ex%dFneq.dat", nits);
      DataSaveASCII(&info, user.f_neq, sn, sm);
#if CHECKERROR == 1
  ...
  for (j = yints; j < yinte; j++) {
    for (i = xints; i < xinte; i++) {
      ...
      for (k = 0; k < user->Q; k++) {
        ...
        sumv = sumv / sumrho;
      for (k = 0; k < user->Q; k++)
        f_neq[j][i].f[k] = -CalFeq(k, sumrho ,sumu, sumv, user) + x[j][i].f[k];
    }
  }
...
PetscErrorCode InitialF(DMDALocalInfo *info, Vec X, Vec Y, Vec Z, Vec W, AppCtx *user)
{
   ...
   PetscScalar    S_xx = 0, S_yy = 0, S_xy = 0, S_yx = 0, S_xx_x = 0, S_yy_y = 0, S_xy_x = 0, S_yx_y = 0;
   float          S_xx_dev[9], S_xy_dev[9], S_yy_dev[9], S_yx_dev[9], sumrho, sumrho_xx_dev, sumrho_yx_dev, sumrho_xy_dev, sumrho_yy_dev;
   Vec            loc_f_neq, loc_x;
  ...
  for (j = yints; j < yinte; j++) {
    for (i = xints; i < xinte; i++) {
      if (j > 0 && j < info->my - 1) {
        ...
        S_xx = 0;S_yy = 0;S_xy = 0;S_yx = 0;S_xx_x = 0;S_yx_y = 0;S_xy_x = 0;S_yy_y = 0;
        sumrho = 0;
        for (k = 0; k < user->Q; k++)
        {
        ...
        for(k = 0; k < user->Q; k++)
        {
          ...
          sumrho_xx_dev = 0;, sumrho_xy_dev = 0;sumrho_yx_dev = 0;sumrho_yy_dev = 0;
          for(l = 0; l < user->Q; l++)
          {
            S_xx_dev[k] += user->eu[l] * user->eu[l] * f_neq[j + user->ev[k]][i + user->eu[k]].f[l];sumrho_xx_dev += u[j + user->ev[k]][i + user->eu[k]].f[l];
            S_xy_dev[k] += user->eu[l] * user->ev[l] * f_neq[j + user->ev[k]][i + user->eu[k]].f[l];sumrho_xy_dev += u;
            S_yx_dev[k] += user->ev[l] * user->eu[l] * f_neq[j + user->ev[k]][i + user->eu[k]].f[l];sumrho_yx_dev += u;
            S_yy_dev[k] += user->ev[l] * user->ev[l] * f_neq[j + user->ev[k]][i + user->eu[k]].f[l];sumrho_yy_dev += u;
          }

          S_xx_x += user->eu[k] * S_xx_dev[k];
          S_xy_x += user->eu[k] * S_xy_dev[k];
          S_yx_y += user->ev[k] * S_yx_dev[k];
          S_yy_y += user->ev[k] * S_yy_dev[k];
        }
        S_xx_x = (user->Nx - 1) / 6.0 * S_xx_x;
        S_yx_y = (user->Nx - 1) / 6.0 * S_yx_y;
        S_xy_x = (user->Nx - 1) / 6.0 * S_xy_x;
        S_yy_y = (user->Nx - 1) / 6.0 * S_yy_y;
        S_xx = -1.0 / user->eta_l * S_xx / sumrho;
        S_xy = -1.0 / user->eta_l * S_xy / sumrho;
        S_yx = -1.0 / user->eta_l * S_yx / sumrho;
        S_yy = -1.0 / user->eta_l * S_yy / sumrho;
        S_xx_x = -1.0 / user->eta_l * S_xx_x;
        S_yx_y = -1.0 / user->eta_l * S_yx_y;
        S_xy_x = -1.0 / user->eta_l * S_xy_x;
        S_yy_y = -1.0 / user->eta_l * S_yy_y;


        x[j][i].f[0] = S_xx * grad_nu[j][i].f[0] + S_xy * grad_nu[j][i].f[1] + (nu[j][i].f[0] - user->eta_l) * S_xx_x + (nu[j][i].f[0] - user->eta_l) * S_xy_x;
        x[j][i].f[1] = S_yy * grad_nu[j][i].f[1] + S_yx * grad_nu[j][i].f[0] + (nu[j][i].f[0] - user->eta_l) * S_yx_y + (nu[j][i].f[0] - user->eta_l) * S_yy_y;
      }
    }
  }
...
PetscErrorCode SetOptions(AppCtx *user)
{
  ...
  user->eta_l = 0.2;                /* viscosity coefficent */
