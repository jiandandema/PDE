...
PetscErrorCode ComputeLfunction(DMDALocalInfo *info, Field **x, Field **f, Field **F, AppCtx *user)
{
    ...
    for (i = xints; i < xinte; i++) {
      if (j > 0 && j < info->my - 1) {
        ...
        for (k = 0; k < user->Q; k++) {
          ...
          //F_k = 3 * user->w[k] * user->eu[k] * user->F_x;
          //F_k = 3 * user->w[k] * user->eu[k] * user->F_x * (1.0 - user->stime / (2.0 * user->tau));
          F_k = user->w[k] * (3 * ((user->eu[k] - sumu) * F[j][i].f[0] + (user->ev[k] - sumv) * F[j][i].f[1]) + 9 * (user->eu[k] * sumu + user->ev[k] * sumv) * (user->eu[k] * F[j][i].f[0] + user->ev[k] * F[j][i].f[1]));
