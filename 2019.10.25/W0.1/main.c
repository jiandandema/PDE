...
PetscErrorCode InitialTAO(DMDALocalInfo *info, AppCtx *user){
  PetscInt        j;
  PetscScalar     nu, phi, n;
...
PetscErrorCode InitialF(DMDALocalInfo *info, AppCtx *user){
  PetscInt        j;
  PetscScalar     grad_nu, n;
  ...
  for (j = info->ys; j < info->ys + info->ym; j++) {
    if (j < 100){
      ...
      user->F[j] = -grad_nu * user->lidvelocity;
      user->F[j] = user->F[j] / user->Re;
    }
    else {
      n = tanh((j * user->step - 2.0 / 3.0) / user->W);
      n = 1 - n*n;
