...
PetscErrorCode Getanalysis(DMDALocalInfo *info, AppCtx *user)
{
  ...
  for (j = info->ys; j < info->ys + info->ym; j++) {
    for (i = info->xs; i < info->xs + info->xm; i++) {
      ...
      xj = 4.0 * ((PetscScalar)(j)) / (info->my - 1);
      x[j][i].f[0] = user->lidvelocity * xj/user->H;                /* real solution:rho = user->rho0; u = user->lidvelocity * 4 * xj; v = 0.0; */
    }
  }
...
PetscErrorCode DataSaveASCIITao(DMDALocalInfo *info, Vec X, char *filename, char *remakefilename)
{
  PetscViewer    dataviewer;                /* http://www.mcs.anl.gov/petsc/petsc-3.2-p7/docs/manualpages/Viewer/PetscViewer.html */
  double         line_solution, mid_solution[360000];
  int            m, n = 360000;
