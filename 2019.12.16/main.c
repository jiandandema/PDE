...
int main(int argc, char **argv)
{
  ...
  DMSetFromOptions(da); 
  DMSetUp(da); 
  DMDASetUniformCoordinates(da, 0.0, 4.0, 0.0, 4.0, 0.0, 4.0);  
...
PetscErrorCode InitialTao(DMDALocalInfo *info, Vec X, AppCtx *user)
{
  ...
  for (j = info->ys; j < info->ys + info->ym; j++) {
        for (i = info->xs; i < info->xs + info->xm; i++) {
                if (j < user->Ny / 2) {
                      n = tanh((j * user->step - user->H / 3.0) / user->W);
                        phi = 0.5 * (-n + 1);
                        nu =  user->rate * phi + 1;
                        x[j][i].f[0] = nu * 3.0 / user->Re;
		}
                else {
                        n = tanh((j * user->step - 2.0*user->H / 3.0) / user->W);
                        phi = 0.5 * (n + 1);
                        nu = user->rate * phi + 1;
                        x[j][i].f[0] = nu * 3.0 / user->Re;
                }
