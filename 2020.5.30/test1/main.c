...
PetscErrorCode SetOptions(AppCtx *user)
{
  ...
  user->nu = user->lidvelocity * 1.0 / user->Re;                /* viscosity coefficent */
  user->tau = 3.0 * user->nu + user->stime / 2.0;                /* maybe the one oder accuracy scheme for dimensionless tau and viscosity coefficent */
  user->Q = 9;
  user->F_x = 8 * user->rho0 * user->nu * user->lidvelocity/(user->H * user->H);
  user->F_y = 0;
  user->ep = 0.0;
