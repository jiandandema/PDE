...
int main(int argc, char **argv)
{
  ...
  sprintf(sm, "exF.dat");
  DataSaveASCIITao(&info, user.F, sn, sm); 
...
PetscErrorCode InitialF(DMDALocalInfo *info, Vec X, AppCtx *user){
  ...
  for (j = info->ys; j < info->ys + info->ym; j++) {
    for (i = info->xs; i < info->xs + info->xm; i++) {
      if (j < user->Ny * 2.0 / 3.0 && j > user->Ny / 3.0) {
        x[j][i].f[0] = 8 * user->lidvelocity * user->rho0 * (1.0/user->Re) / pow(user->H,2);
      }
      else{
        x[j][i].f[0] = 8 * user->lidvelocity * user->rho0 * (50.0/user->Re) / pow(user->H,2);
      }
      for (k = 1; k < user->Q; k++) {
        x[j][i].f[k]  = x[j][i].f[0];
      }
